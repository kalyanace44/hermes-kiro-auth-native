...
def start_compaction_proxy():
    ...
    def run_proxy():
        try:
            server = ReusableHTTPServer(("127.0.0.1", PORT), KiroCompactionProxy)
            logger.info(f"[Kiro] Compaction proxy listening on http://127.0.0.1:{PORT} → Go gateway on {INTERNAL_PORT}")
            server.serve_forever()
        except OSError as e:
            if e.errno == 48:  # Address already in use
                logger.warning(f"[Kiro] Port {PORT} already in use — proxy likely running from a previous instance, skipping.")
            else:
                raise
